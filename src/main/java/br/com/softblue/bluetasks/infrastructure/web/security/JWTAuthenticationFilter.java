...
// Permite expor a URL /login automaticamente associada a esse filtro.
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	// Processa a tentativa de autenticacao
	@Override
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
			throws AuthenticationException {
			
		try {
			// Bind do JSON para AppUser com Jackson
			ObjectMapper mapper = new ObjectMapper();
			AppUser appUser = mapper.readValue(request.getInputStream(), AppUser.class);
			
			// Gera Token de autenticacao e retorna
	...
	// Processa se a autenticacao der certo, criar o token e retorna para o cliente
	@Override
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
			Authentication authResult) throws IOException, ServletException {
		
		// Pegando Detalhes do usuario autenticado
		UserDetailsImpl userDetails = (UserDetailsImpl) authResult.getPrincipal();
		
		// Gerando Token do Cliente
		...
		// Retornando token no header da requisicao
